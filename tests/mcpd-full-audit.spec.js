...
const pagesToAudit = [
  ...
  { name: 'law-lookup-public-defecation', path: '/legal/search?q=pooping%20in%20the%20street&source=ALL&state=GA' },
...
async function auditPage(page, target, viewportName, events) {
  ...
  try {
    ...
    if (target.name === 'reports-list' || target.name === 'dashboard') {
      if (/Stress Report\s+\d+|Mock Stress Report\s+\d+|Test Report\s+\d+/i.test(text)) {
        record.notes.push('Stress/test report visible to normal user');
      }
    }
    if (target.name === 'law-lookup-public-defecation') {
      if (/shoplifting|exchange\/px theft/i.test(text)) {
        record.notes.push('Unrelated shoplifting result visible for public defecation query');
      }
      if (!/disorderly|public indecency|no strong approved source found/i.test(text)) {
        record.notes.push('Expected public-conduct law path not visible');
      }
    }
    if (target.name === 'mobile-home') {
      if (/Shift Workflow|Command Radar|Operations Board|Paperwork Navigator/i.test(text)) {
        record.notes.push('Desktop dashboard clutter visible on mobile home');
      }
    }
    const badEmptyLinks = await page.locator('a[href=""], a[href="#"]').evaluateAll((links) => (
      links.filter((link) => {
    ...
    if (badEmptyLinks > 0) record.notes.push(`${badEmptyLinks} empty/hash links visible`);
    try {
