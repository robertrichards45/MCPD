(function () {
    ...
    return `
        <section class="mobile-id-scan-launcher">
          <div class="mobile-id-scan-copy">
            <strong>ID scanner</strong>
            <p>Scan the PDF417 barcode or paste scan text. Manual correction stays available.</p>
          </div>
          <button class="mobile-scan-button is-camera" type="button" data-id-scan-camera>Open Live ID Scanner</button>
          <a class="mobile-scan-button is-secure" href="#" data-id-secure-launch hidden>Open Secure Scanner</a>
          <button class="mobile-outline-button" type="button" data-id-scan-file-button>Photo Fallback</button>
  ...
  function factValueMap(state) {
    ...
    facts.forEach((entry) => {
      ...
      values[entry.id] = String(entry.value || '');
    });
    return values;
  }
  ...
  function PersonEditorPage(root, urls) {
    ...
    const sourceInput = form ? form.querySelector('input[name="source"]') : null;
        ...
        const secureLaunch = root.querySelector('[data-id-secure-launch]');
        const scanFileButton = root.querySelector('[data-id-scan-file-button]');
        ...
        const secureScannerUrl = () => {
          const host = window.location.hostname || '127.0.0.1';
          const currentPort = window.location.port || '';
          const port = currentPort === '8092'
            ? ':8091'
            : (currentPort && currentPort !== '80' && currentPort !== '443' ? `:${currentPort}` : '');
          return `https://${host}${port}${window.location.pathname}${window.location.search || ''}`;
        };

        const syncSecureScannerLaunch = () => {
          if (!secureLaunch) return;
          if (window.isSecureContext) {
            secureLaunch.hidden = true;
            secureLaunch.removeAttribute('href');
            return;
          }
          secureLaunch.hidden = false;
          secureLaunch.setAttribute('href', secureScannerUrl());
        };

        syncSecureScannerLaunch();
      ...
      if (scanApply && form) {
        ...
        const openCaptureFallback = () => {
          ...
          if (scanStatus) {
            scanStatus.textContent = 'Opening photo fallback. Live scanner remains the preferred path.';
          }
          scanFile.click();
        };
        const startLiveScanner = async () => {
          ...
          try {
            ...
            if (!window.isSecureContext) {
              ...
              syncSecureScannerLaunch();
              if (secureLaunch) secureLaunch.focus();
              stopLiveScanner();
              return;
            }
            if (!zxing || typeof zxing.BrowserPDF417Reader !== 'function') {
              scanStatus.textContent = 'Live scanner library is unavailable. Use manual entry, paste scan text, or Photo Fallback.';
              stopLiveScanner();
              return;
            }
            if (!navigator.mediaDevices || typeof navigator.mediaDevices.getUserMedia !== 'function') {
              scanStatus.textContent = 'Camera scanner is not available in this browser. Use manual entry, paste scan text, or Photo Fallback.';
              stopLiveScanner();
              return;
            }
            ...
            scanStatus.textContent = 'Scanner could not start. Use manual entry, paste scan text, or Photo Fallback.';
            stopLiveScanner();
          } finally {
