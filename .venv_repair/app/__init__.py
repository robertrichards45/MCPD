...
def ensure_schema():
    ...
    if 'report' in table_names:
        ...
        if 'call_type_slug' not in report_columns:
            _safe_schema_execute("ALTER TABLE report ADD COLUMN call_type_slug VARCHAR(120)")
        if 'facts_text' not in report_columns:
            _safe_schema_execute("ALTER TABLE report ADD COLUMN facts_text TEXT")
        if 'narrative_text' not in report_columns:
            _safe_schema_execute("ALTER TABLE report ADD COLUMN narrative_text TEXT")
        if 'blotter_text' not in report_columns:
            _safe_schema_execute("ALTER TABLE report ADD COLUMN blotter_text TEXT")
        if 'paperwork_json' not in report_columns:
            _safe_schema_execute("ALTER TABLE report ADD COLUMN paperwork_json TEXT")
        if 'officer_notes' not in report_columns:
            _safe_schema_execute("ALTER TABLE report ADD COLUMN officer_notes TEXT")
