...
def test_call_type_manager_adds_rules_and_mobile_consumes_them(monkeypatch, tmp_path):
    ...
    try:
        ...
        try:
            ...
            assert 'Incident Intake Builder' in html
            assert 'Build the call types officers see in the field.' in html
            assert 'Preview Mobile Intake' in html
        finally:
            response.close()
        ...
        try:
            body = response.get_data(as_text=True)
            assert response.status_code == 200
            assert 'Desktop Incident Intake' in body
            assert 'Noise Complaint' in body
            assert 'Manage Scenarios' in body
        finally:
            response.close()
    finally:
        _dispose_app(app)
