...
from app.routes import reference, reports
...
def test_report_workflow_saves_facts_narrative_and_paperwork(monkeypatch, tmp_path):
    app = _isolated_app(monkeypatch, tmp_path)
    try:
        client = _logged_in_client(app)
        response = client.post('/reports/new', data={'title': 'Domestic disturbance test'}, follow_redirects=False)
        assert response.status_code in {302, 303}
        report_id = int(response.headers['Location'].rstrip('/').split('/')[-1])

        response = client.post(
            f'/reports/{report_id}/workflow',
            data={
                'call_type_slug': 'domestic-disturbance',
                'facts_text': 'Victim stated the suspect pushed her and took her phone.',
                'narrative_text': 'On the listed date, the victim stated the suspect pushed her and took her phone.',
                'blotter_text': 'Domestic disturbance report generated for review.',
                'paperwork': ['Narrative', 'Domestic Supplemental', 'Voluntary Statement'],
            },
        ...
        with app.app_context():
            ...
            assert report.call_type_slug == 'domestic-disturbance'
            assert 'pushed her' in report.facts_text
            assert 'listed date' in report.narrative_text
            assert json.loads(report.paperwork_json) == ['Narrative', 'Domestic Supplemental', 'Voluntary Statement']
            owner = db.session.get(User, report.owner_id)
            owner.email = 'officer@example.test'
            db.session.commit()
        ...
        assert 'Describe what happened' in html
        assert 'Authority Search' in html
        assert 'Preview / Print' in html
        assert 'Email Packet' in html

        preview = client.get(f'/reports/{report_id}/packet/preview')
        assert preview.status_code == 200
        assert preview.headers['Content-Type'].startswith('application/pdf')
        assert len(PdfReader(BytesIO(preview.data)).pages) >= 1

        download = client.get(f'/reports/{report_id}/packet/download')
        assert download.status_code == 200
        assert 'attachment' in download.headers.get('Content-Disposition', '').lower()

        monkeypatch.setattr(reports, '_smtp_send_report_packet', lambda *args, **kwargs: (True, 'sent'))
        emailed = client.post(f'/reports/{report_id}/packet/email', follow_redirects=False)
        assert emailed.status_code in {302, 303}
    finally:
        _dispose_app(app)
