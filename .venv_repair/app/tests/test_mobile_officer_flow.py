...
def test_mobile_officer_flow_pages_include_updated_mobile_assets():
    ...
    try:
        ...
        runtime = _text_response(client.get('/static/mobile/incident-core.js?v=2026-04-30-scanner-packet-1'))
        ...
        more = _text_response(client.get('/mobile/more'))
        assert 'Scenario Builder' in more
        assert 'Desktop Dashboard' in more
        facts_page = _text_response(client.get('/mobile/incident/facts'))
        assert 'Facts</span>' in facts_page
        assert 'Incident</span>' in facts_page
        assert 'data-mobile-incident-page="selected-forms"' in forms
        assert '2026-04-30-scanner-packet-1' in domestic
        assert '/static/vendor/zxing-browser.min.js?v=2026-04-30-scanner-packet-1' in domestic
        assert 'mobile-domestic-schema-data' in domestic
        assert 'data-id-scan-raw' in runtime
        assert 'data-id-scan-file' in runtime
        assert 'data-id-scan-file-button' in runtime
        assert 'data-id-secure-launch' in runtime
        assert 'https://${host}${port}${window.location.pathname}' in runtime
        assert 'Photo Fallback' in runtime
...
def test_mobile_runtime_uses_guided_statement_and_domestic_flows():
    ...
    try:
        ...
        assert 'function factValueMap(state)' in runtime
        assert runtime.index('function factValueMap(state)') < runtime.index('function FactsCapturePage(root, urls)')
        ...
        assert 'function readZxingResultText(result)' in runtime
        assert runtime.index('function readZxingResultText(result)') < runtime.index('function detectBarcodeTextFromImageFile(file)')
        assert runtime.index('function readZxingResultText(result)') < runtime.index('decodeFromVideoDevice')
    finally:
        _dispose_app(client.application)
