...
def _write_xfa_sectioned_pdf(
    ...
    c.setSubject('Browser-compatible render for Adobe dynamic XFA form fields')
    ...
    c.roundRect(left - 2, y - 7, width - (left * 2) + 4, 60, 4, stroke=0, fill=1)
    c.setFillColorRGB(0.06, 0.12, 0.18)
    c.setFont('Helvetica-Bold', 8)
    c.drawString(left + 4, y + 38, 'Render Certification')
    c.setFont('Helvetica', 7)
    certification_lines = [
        f'Original XFA fields detected: {len(fields)} | rendered from schema sections plus discovered PDF fields.',
        'Adobe dynamic XFA page art cannot be displayed by most browser PDF viewers; this output removes the Adobe "Please wait" shell.',
        'Use the official Adobe PDF source for exact dynamic page art; use this packet for browser-safe review, print, download, and email.',
    ]
    for index, line in enumerate(certification_lines):
        c.drawString(left + 4, y + 26 - (index * 10), line[:132])
    ...
    return {
        ...
        'page_art_exact': False,
        'browser_safe': True,
        'render_certification': 'XFA fields rendered into a browser-safe packet; exact Adobe dynamic page art requires the official Adobe XFA source/viewer.',
    }
