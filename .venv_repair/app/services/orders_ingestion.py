...
def _upsert_order_document(payload: dict) -> tuple[bool, bool]:
    ...
    if existing:
        ...
        if source_url and source_url not in (existing.source_group or ""):
            ...
            changed = True
        if changed:
            existing.last_indexed_at = _utcnow_naive()
...
def run_official_orders_ingestion(max_new: int = 20, fetch_limit: int = 120) -> dict:
    _ensure_paths()
    state = _load_state()
    state["last_run_utc"] = _utcnow_naive().isoformat()
    inserted = 0
    updated = 0
    scanned = 0
    errors: list[str] = []
    ...
    try:
        ...
        for candidate in candidates:
            ...
            if created:
                ...
                updated += 1
        db.session.commit()
        state["last_success_utc"] = _utcnow_naive().isoformat()
        state["last_error"] = ""
    ...
    state["next_due_utc"] = (_utcnow_naive() + timedelta(hours=24)).isoformat()
    _save_state(state)
