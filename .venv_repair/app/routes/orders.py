...
def _write_seed_text_file(slug, text_content):
    safe_slug = re.sub(r'[^a-z0-9\-]+', '-', slug.lower()).strip('-') or f"order-{int(_utcnow_naive().timestamp())}"
...
def library():
    ...
    if request.method == 'POST':
            ...
            for doc in docs:
                if doc.file_path and os.path.exists(doc.file_path):
                    ...
                    if extracted and extracted != (doc.extracted_text or ''):
                        ...
                        doc.last_indexed_at = _utcnow_naive()
                        updated += 1
            if updated:
                db.session.commit()
        ...
        stamped_name = f"{int(_utcnow_naive().timestamp())}-{original_name}"
...
def view_order(order_id):
    ...
    if document.revision_date:
        age_days = (_utcnow_naive() - document.revision_date).days
        if age_days > 365 * 2:
            freshness_warning = 'source currency requires verification'
    elif document.issue_date:
        age_days = (_utcnow_naive() - document.issue_date).days
        if age_days > 365 * 2:
            freshness_warning = 'source currency requires verification'
    elif document.uploaded_at:
        age_days = (_utcnow_naive() - document.uploaded_at).days
        if age_days > 365 * 2:
            freshness_warning = 'source currency requires verification'
    ...
    if not document.extracted_text and file_exists:
        ...
        document.last_indexed_at = _utcnow_naive()
