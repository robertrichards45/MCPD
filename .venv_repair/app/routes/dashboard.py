...
def _dashboard_snapshot():
    ...
    return {
        ...
        'recent_forms': (
            SavedForm.query
            .filter_by(officer_user_id=current_user.id)
            .order_by(SavedForm.updated_at.desc(), SavedForm.id.desc())
            .limit(5)
            .all()
        ),
    }
...
def dashboard():
    ...
    return _no_store_response(
        render_template(
            ...
            dashboard_recent_reports=snapshot['recent_reports'],
            dashboard_recent_forms=snapshot['recent_forms'],
        )
    )
