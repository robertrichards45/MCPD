...
- Added render certification metadata/block to XFA-compatible PDFs so reviewers can tell the packet is browser-safe field coverage, not an exact Adobe dynamic XFA page-art flatten.
...
## 2026-04-30 UI / Workflow Verification Addendum

- Full headed Playwright audit now covers dashboard, mobile home, report intake, legal lookup scenarios, forms, saved work, orders, training, personnel, scanner/person entry, scenario builder, and navigator editor.
- Local audit result: no blocking page-load items captured after the scanner URL and long-screenshot fallback were repaired.
- Form accuracy status did not change in this UI pass: official forms remain the source of truth, but exact certification for XFA page-art flattening is still limited as noted above.
