(function () {
  ...
  function getVoiceSpeed() {
    try {
      return localStorage.getItem(STORAGE_SPEED) || 'fast';
    } catch (e) {
      return 'fast';
    }
  }
  ...
  function speakSummary(text, onDone) {
    ...
    if (onDone) {
      var waitMs = Math.min(6000, Math.max(900, shortText.length * 35));
      setTimeout(onDone, waitMs);
    }
  }
