...
def ensure_npc_mind(state, actor, run_context=None):
    ...
    row.setdefault('reliability', rng.choice(RELIABILITY))
    ...
    row.setdefault('incorrect_beliefs', [])
...
def respond(state, actor, question, allowed_facts, visible_facts, fallback, run_context=None, officer_actions=None):
    """Return an in-character response bounded by scenario truth.

    The model receives only facts the actor may know plus facts already visible in
    the run. Hidden scenario state is never included in the prompt.
    """
    ...
    allowed_blob = '\n'.join(allowed_facts + visible_facts)
