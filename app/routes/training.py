...
def training_upload():
    ...
    if request.method == 'POST':
        ...
        filename = f"{int(datetime.utcnow().timestamp())}-{file.filename}".replace(' ', '_')
...
def training_sign(roster_id):
    ...
    sig_name = f"sig-{roster_id}-{current_user.id}-{int(datetime.utcnow().timestamp())}.png"
    ...
    append_signature_page(roster.file_path_original, sig_path, current_user.display_name, datetime.utcnow().isoformat(), compiled)
