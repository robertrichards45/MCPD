...
bp = Blueprint('fto_scenario_lab', __name__, url_prefix='/fto-center/scenario-lab')

# Scenario Lab is intentionally synthetic and advisory. It does not write DOR
# ratings, advance a trainee, or make employment/training decisions.
SCENARIOS = {
    'S001': {
        ...
        'stages': [
            {
                'name': 'Arrival',
                'prompt': 'You are arriving at Building 7130. What do you do first?',
                'reveal': 'A staff member meets you outside and says the subject is still inside, is yelling, and has refused multiple requests to leave. No weapon has been reported.',
            },
            {
                'name': 'Initial Contact',
                'prompt': 'You now have the staff member’s initial information. How do you approach and handle first contact?',
                'reveal': 'The subject is argumentative but keeps his hands visible. He says he has a right to remain because he was invited earlier. Staff says the invitation was revoked after the disturbance began.',
            },
            {
                'name': 'Investigation',
                'prompt': 'What facts do you need to establish before deciding what action is appropriate?',
                'reveal': 'A second employee says they personally heard the subject being told to leave. Video is available. The subject begins to calm down and says he will leave if the officer explains what happens next.',
            },
            {
                'name': 'Disposition',
                'prompt': 'Describe your disposition, notifications, and documentation based only on the facts developed.',
                'reveal': 'Scenario complete. The instructor should now review whether the trainee developed the facts, used appropriate communication and safety practices, and clearly articulated the final disposition.',
            },
        ],
    },
    'S002': {
...
# These are coaching areas aligned to concepts in the FTP Standard Evaluation
# Guidelines. They are NOT official DOR ratings and intentionally do not assign
# the manual's 1/4/7 rating values automatically.
PRACTICE_AREAS = [
...
SESSION_KEY = 'fto_scenario_lab_v2'
...
def _coaching_summary(state):
    ...
    for label, _terms in PRACTICE_AREAS:
        ...
        if count >= 4:
            ...
            level = 'needs-work'
        rows.append({'area': label, 'count': count, 'status': status, 'level': level})

    discussion = [
        row['area'] for row in rows if row['level'] == 'needs-work'
    ][:5]
    return {
        'areas': rows,
        'discussion': discussion,
        'notice': (
            'Scenario Lab is a coaching aid. These are text-based practice cues only, not SEG/DOR ratings. '
            'The assigned FTO must evaluate actual performance using the approved FTP standards.'
        ),
    }


def seed_legacy_session(scenario_id, response_text=None):
    """Move the older one-box Scenario Lab form into the interactive flow."""
    sid = _valid_scenario(scenario_id)
    state = _new_state(sid)
    if _normalize(response_text):
...
def scenario_lab():
    ...
    if not isinstance(state, dict) or state.get('scenario_id') not in SCENARIOS:
        state = _new_state(requested_id)
    elif request.method == 'GET' and request.args.get('scenario_id') in SCENARIOS and requested_id != state.get('scenario_id'):
        state = _new_state(requested_id)
    ...
    if request.method == 'POST':
        ...
        if action == 'reset':
            ...
            session.modified = True
            return redirect(url_for('reports.sentinel.fto_scenario_lab.scenario_lab', scenario_id=requested_id))
        ...
        if action == 'finish':
            if int(state.get('turn', 0)) < 1:
                ...
                session[SESSION_KEY] = state
                session.modified = True
            return redirect(url_for('reports.sentinel.fto_scenario_lab.scenario_lab', scenario_id=state['scenario_id']))
        ...
        if action == 'submit':
            ...
            if turn >= len(stages):
                flash('All scenario stages are complete. Finish the scenario to review coaching cues.', 'info')
                return redirect(url_for('reports.sentinel.fto_scenario_lab.scenario_lab', scenario_id=state['scenario_id']))
