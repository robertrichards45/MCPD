...
def cleo_save(page_key):
    ...
    row = CleoFormData.query.filter_by(user_id=current_user.id, page_key=page_key).first()
    if not row:
        row = CleoFormData(user_id=current_user.id, page_key=page_key, data_json=json.dumps(data), updated_at=datetime.utcnow())
        db.session.add(row)
    else:
...
def cleo_layout_save(page_key):
    ...
    row = CleoFormLayout.query.filter_by(page_key=page_key).first()
    if not row:
        row = CleoFormLayout(page_key=page_key, layout_json=json.dumps(layout), updated_at=datetime.utcnow())
        db.session.add(row)
    else:
...
def cleo_file_upload(page_key):
    ...
    for file in files:
        ...
        filename = f"{current_user.username}-{page_key}-{int(datetime.utcnow().timestamp())}{ext}"
...
def cleo_report_page_save(report_id, page_key):
    ...
    row = CleoReportPage.query.filter_by(report_id=report_id, page_key=page_key).first()
    if not row:
        row = CleoReportPage(report_id=report_id, page_key=page_key, data_json=json.dumps(data), updated_at=datetime.utcnow())
        db.session.add(row)
    else:
        row.data_json = json.dumps(data)
        row.updated_at = datetime.utcnow()
