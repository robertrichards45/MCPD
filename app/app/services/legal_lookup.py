...
def _scenario_supported_adjustment(profile: SearchEntryProfile, analysis: QueryAnalysis) -> tuple[float, tuple[str, ...], tuple[str, ...]]:
    ...
    base_context = has(r'\b(base|gate|px|exchange|barracks|marine|military installation|federal installation|restricted area|mclb)\b')
    barred_entry = has(r'barred|told not to (?:come back|return)|not to return|came back|returned|reenter|re-enter|restricted area|without permission|no permission|after hours|unauthorized')
    drug_context = has(r'\b(weed|marijuana|cannabis|drug|controlled substance|narcotic|pills?)\b')
    vehicle_context = has(r'\b(vehicle|car|truck|gate|traffic stop|through the gate)\b')
    domestic_context = has(r'\b(victim|boyfriend|girlfriend|husband|wife|spouse|domestic|argument|dating partner|family)\b')
    force_context = has(r'\b(push(?:ed)?|shove(?:d)?|grab(?:bed)?|hit|struck|slap(?:ped)?|fight(?:ing)?)\b')
    property_context = has(r'\b(phone|property|merchandise|items?|px|exchange|store|points? of sale|took|stole|shoplift)\b')
    order_context = has(r'\b(direct order|lawful order|nco|sergeant|refused order|would not follow|disobey)\b')
    threat_context = has(r'\b(threat(?:ened)?|kill|text message|by text|harass(?:ing)? message)\b')
    building_entry_context = has(r'\b(building|warehouse|after hours|inside|entered|remained|without permission|no permission)\b')
    intox_barracks_context = has(r'\b(drunk|intoxicated|alcohol)\b') and has(r'\b(barracks|marine|service member)\b')
    theft_from_px = has(r'\b(px|exchange|store|merchandise|points? of sale|shoplift)\b') and has(r'\b(took|stole|passed|conceal|deprive|property|items?)\b')

    if code == '18 USC 1382':
        if base_context and barred_entry:
            score += 52
            reasons.extend(('matched fact: reentered or remained after warning/without permission', 'matched location: military installation'))
            terms.extend(('base', 'barred', 'restricted area'))
        elif barred_entry and not base_context:
            score -= 48
            reasons.append('lowered: federal installation facts not present')
        elif drug_context or theft_from_px or intox_barracks_context:
            score -= 58
            reasons.append('lowered: installation entry offense not supported by the main facts')
    if code in {'OCGA 16-13-75', 'OCGA 16-13-30', 'Article 112a'} and drug_context:
        score += 46
        reasons.append('matched fact: controlled substance or marijuana present')
        if vehicle_context:
            reasons.append('matched location/context: vehicle or gate')
        if code == 'Article 112a' and base_context:
            score += 14
            reasons.append('matched jurisdiction: military/base drug context')
        terms.extend(('marijuana', 'controlled substance', 'vehicle'))
    if code in {'OCGA 16-5-23', 'OCGA 16-5-23.1', 'Article 128b'} and domestic_context and force_context:
        score += 44
        reasons.append('matched fact: domestic/intimate-partner physical contact')
        if code == 'Article 128b' and base_context:
            score += 12
            reasons.append('matched jurisdiction: UCMJ domestic violence path')
        terms.extend(('domestic', 'pushed', 'grabbed'))
    if code in {'OCGA 16-8-2', 'Article 121'} and property_context and has(r'\b(took|stole|grabbed|withheld|appropriat)\b'):
        score += 30
        reasons.append('matched fact: property was taken or withheld')
        terms.extend(('property', 'took'))
    if code in {'OCGA 16-8-14', 'OCGA 16-8-2', '18 USC 641', 'Article 121'} and theft_from_px:
        score += 50
        reasons.append('matched fact: PX/store property passed points of sale')
        if code in {'18 USC 641', 'Article 121'}:
            reasons.append('matched jurisdiction: government/military property context')
        terms.extend(('px', 'points of sale', 'property'))
    if code in {'Article 92', 'Article 91'} and order_context:
        score += 52
        reasons.append('matched fact: service member refused or disobeyed an order')
        terms.extend(('direct order', 'nco'))
    if code in {'OCGA 16-7-21', 'OCGA 16-7-22'} and building_entry_context:
        score += 46
        reasons.append('matched fact: entered/remained in building without authority')
        terms.extend(('building', 'without permission'))
    if code in {'18 USC 2261A', 'OCGA 16-5-102', 'Article 130'} and threat_context:
        score += 44
        reasons.append('matched fact: threat communicated by text/electronic message')
        terms.extend(('threatened', 'text message'))
    if code in {'Article 112', 'Article 128', 'OCGA 16-11-39'} and intox_barracks_context:
        score += 38
        reasons.append('matched fact: intoxicated service member/barracks disturbance')
        if force_context and code == 'Article 128':
            score += 18
            reasons.append('matched fact: fighting/assaultive conduct')
        terms.extend(('drunk', 'barracks', 'fighting'))

    # Explicitly de-emphasize common keyword traps unless the full scenario supports them.
    if code == 'OCGA 40-6-391' and has(r'\b(drunk|intoxicated|alcohol)\b') and not has(r'\b(driv|vehicle|car|truck|traffic stop|roadway|dui|less safe)\b'):
        score -= 64
        reasons.append('lowered: DUI requires driving/vehicle facts not present')
    if code == 'OCGA 40-6-241' and threat_context and not has(r'\b(driv|roadway|traffic|vehicle)\b'):
        score -= 58
        reasons.append('lowered: texting-while-driving not supported by threat facts')
    if code == 'Article 120' and force_context and not has(r'\b(sex|sexual|rape|penetrat|nonconsensual)\b'):
        score -= 72
        reasons.append('lowered: sexual-offense facts not present')
    if code == 'OCGA 16-6-22' and not has(r'\b(sex|sexual|rape|penetrat|nonconsensual)\b'):
        score -= 72
        reasons.append('lowered: sexual-battery facts not present')
    if code == 'OCGA 16-5-24' and not has(r'\b(serious injury|disfigure|broken|weapon|aggravated|loss of|fracture)\b'):
        score -= 34
        reasons.append('lowered: aggravated-injury facts not present')
    if code == 'Article 128b' and not domestic_context:
        score -= 44
        reasons.append('lowered: domestic relationship facts not present')
    if code == 'OCGA 40-6-270' and not has(r'\b(crash|collision|struck|hit and run|left the scene|accident)\b'):
        score -= 52
        reasons.append('lowered: hit-and-run/crash facts not present')
    if code == 'OCGA 40-6-221' and not has(r'\b(handicap|disabled parking|placard|parking)\b'):
        score -= 52
        reasons.append('lowered: disabled-parking facts not present')
    if code == '18 USC 922(g)' and not has(r'\b(firearm|gun|weapon|ammunition)\b'):
        score -= 52
        reasons.append('lowered: firearm facts not present')
    if code == 'OCGA 16-13-30.4' and not has(r'\b(internet|online|website|computer)\b'):
        score -= 44
        reasons.append('lowered: internet drug distribution facts not present')
    if code == 'OCGA 40-6-390' and not has(r'\b(reckless|unsafe|speed|disregard|swerv|weav|aggressive)\b'):
        score -= 44
        reasons.append('lowered: reckless-driving facts not present')
    if code == '18 USC 1361' and not has(r'\b(damaged|destroyed|willful injury|vandal|broke)\b'):
        score -= 34
        reasons.append('lowered: property-damage facts not present')
    if code == 'Article 108' and theft_from_px and not has(r'\b(damaged|destroyed|lost|sold|disposed military property)\b'):
        score -= 30
        reasons.append('lowered: larceny path fits better than military-property damage/loss')

    return score, tuple(reasons), tuple(_ordered_unique(terms))
...
def _score_profile(profile: SearchEntryProfile, analysis: QueryAnalysis, strict_gating: bool) -> tuple[float, tuple[str, ...], tuple[str, ...]]:
    ...
    scenario_score, scenario_reasons, scenario_terms = _scenario_supported_adjustment(profile, analysis)
    if scenario_score:
        score += scenario_score
        reasons.extend(scenario_reasons)
        matched_terms.extend(scenario_terms)
    ...
    final_reasons = _ordered_unique(reasons)[:10]
    final_terms = _ordered_unique(matched_terms)[:12]
...
def _required_codes_for_query(analysis: QueryAnalysis, source: str) -> tuple[str, ...]:
    ...
    if re.search(r'aggravated sexual battery|sexual battery|penetration with a foreign object|foreign object of another', normalized):
    ...
    vehicle_impairment_context = (
        re.search(r'\bdui\b|refus\w+.*(?:breath|blood|test)|implied consent|less safe|weav', normalized)
        or (
            re.search(r'intoxicat|impaired|drunk|alcohol', normalized)
            and re.search(r'driv|vehicle|car|truck|traffic stop|roadway|behind the wheel', normalized)
        )
    )
    if vehicle_impairment_context:
    ...
    if re.search(r'\bmarijuana\b|\bweed\b|\bcannabis\b', normalized):
        ...
        if re.search(r'\b(base|gate|barracks|marine|service member|military)\b', normalized):
            required.append('Article 112a')
    ...
    if re.search(r'building|warehouse|after hours|inside .*without permission|entered .*without permission|did not have permission|no permission', normalized):
        required.extend(['OCGA 16-7-21', 'OCGA 16-7-22'])
    if re.search(r'shoplifting|shoplift|retail theft|merchandise from store|from px|from the px|points? of sale|exchange', normalized):
        required.extend(['OCGA 16-8-14', 'OCGA 16-8-2', '18 USC 641', 'Article 121'])
    if re.search(r'\bpushed\b|\bslapped\b|\bgrabbed\b|\bshoved\b|\bhit\b', normalized):
        required.extend(['OCGA 16-5-23', 'OCGA 16-5-20'])
        if any(term in normalized for term in ('spouse', 'wife', 'husband', 'boyfriend', 'girlfriend', 'dating partner', 'domestic', 'family violence')):
            required.extend(['OCGA 16-5-23.1', 'Article 128b'])
    if re.search(r'violent injury|reasonable apprehension|placing another in reasonable|attempt(?:ed)? .*injur', normalized):
        required.append('OCGA 16-5-20')
    if re.search(r'insulting/provoking contact|insulting or provoking contact|intentional physical harm|physical harm', normalized):
        required.append('OCGA 16-5-23')
    if re.search(r'depriv(?:ing|e) body member|body member|render(?:ing)? .*member useless|serious disfigure|disfigurement|maliciously causing bodily harm', normalized):
        required.append('OCGA 16-5-24')
    if re.search(r'offers? to do bodily harm|attempts? .*bodily harm|bodily harm', normalized):
        required.append('Article 128')
    ...
    if re.search(r'aggravated identity theft|means of identification|knowingly transferring possessing or using|transferring possessing or using', normalized):
        required.append('18 USC 1028A')
    if re.search(r'government property|from px|from the px|exchange', normalized):
        required.extend(['18 USC 641', 'Article 121'])
    if re.search(r'federal vandalism|vandalism|damaged government property|destroyed government property', normalized):
        required.append('18 USC 1361')
    ...
    if re.search(r'\b(kidnap|kidnapping|abduct|abducting|abduction|stealing away|held against will)\b', normalized):
        required.append('OCGA 16-5-40')
    if re.search(r'\b(carrying weapons?|weapon carried|firearm carried|carrying firearm)\b', normalized):
        required.append('OCGA 16-11-126')
    if re.search(r'felon in possession|prohibited person|firearm possession by felon', normalized):
        required.append('18 USC 922(g)')
    if re.search(r'exchanging information|rendering aid|left (?:the )?scene|hit and run', normalized):
        required.append('OCGA 40-6-270')
...
def _forbidden_codes_for_query(analysis: QueryAnalysis) -> tuple[str, ...]:
    normalized = analysis.normalized_query
    # Direct citation lookups should never be filtered by narrative-only
    # suppressors. If an officer/admin types "OCGA 16-5-20" or "Article 92",
    # they are asking for that reference, not a plain-language scenario match.
    if analysis.ocga_code or analysis.ocga_prefix or analysis.article_number or re.search(r'\b\d+\s+usc\b', normalized):
        return ()
    forbidden: list[str] = []
    if 'prescription' in normalized and 'pill' in normalized and 'forg' not in normalized and 'fake prescription' not in normalized:
    ...
    if re.search(r'\b(drunk|intoxicated|alcohol)\b', normalized) and not re.search(r'\b(driv|vehicle|car|truck|traffic stop|roadway|dui|less safe)\b', normalized):
        forbidden.append('OCGA 40-6-391')
    if re.search(r'threat(?:ened)?|kill|text message|by text', normalized) and not re.search(r'\b(driv|roadway|traffic|vehicle)\b', normalized):
        forbidden.append('OCGA 40-6-241')
    if not re.search(r'\b(sex|sexual|rape|penetrat|penetration|foreign object|nonconsensual)\b', normalized):
        forbidden.extend(['Article 120', 'OCGA 16-6-22'])
    if not re.search(r'\b(serious injury|disfigure|disfigurement|broken|weapon|aggravated|loss of|fracture|body member|member useless|maliciously causing bodily harm)\b', normalized):
        forbidden.append('OCGA 16-5-24')
    if not re.search(r'\b(crash|collision|struck|hit and run|left (?:the )?scene|accident|exchanging information|rendering aid)\b', normalized):
        forbidden.append('OCGA 40-6-270')
    if not re.search(r'\b(handicap|disabled parking|placard|parking)\b', normalized):
        forbidden.append('OCGA 40-6-221')
    if not re.search(r'\b(firearm|gun|weapon|weapons|ammunition|felon in possession|prohibited person)\b', normalized):
        forbidden.extend(['18 USC 922(g)', '18 USC 930', 'OCGA 16-11-126', 'OCGA 16-11-127'])
    if not re.search(r'\b(internet|online|website|computer)\b', normalized):
        forbidden.append('OCGA 16-13-30.4')
    if not re.search(r'\b(reckless|unsafe|speed|disregard|swerv|weav|aggressive)\b', normalized):
        forbidden.append('OCGA 40-6-390')
    if not re.search(r'\b(false name|fake id|identity|stolen id|identification|means of identification|transferring possessing or using)\b', normalized):
        forbidden.append('18 USC 1028A')
    if not re.search(r'\b(kidnap|kidnapping|abduct|abducting|abduction|stealing away|forced into|held against will|would not let .* leave|blocked .* leave)\b', normalized):
        forbidden.append('OCGA 16-5-40')
    if not re.search(r'\b(push|shove|grab|hit|struck|slap|fight|assault|battery|threatened to hit|violent injury|reasonable apprehension|provoking contact|physical harm|bodily harm)\b', normalized):
        forbidden.extend(['Article 128', 'OCGA 16-5-20', 'OCGA 16-5-21', 'OCGA 16-5-23'])
    if not re.search(r'\b(damaged|destroyed|willful injury|vandal|vandalism|broke)\b', normalized):
        forbidden.append('18 USC 1361')
    if (
        re.search(r'\b(weed|marijuana|cannabis|drug|controlled substance|px|exchange|points? of sale|drunk|barracks|fight)\b', normalized)
        and not re.search(r'barred|told not to (?:come back|return)|not to return|came back|returned|reenter|re-enter|restricted area|without permission|no permission|unauthorized', normalized)
    ):
        forbidden.append('18 USC 1382')
    if (
        re.search(r'building|warehouse|after hours|inside|without permission|no permission', normalized)
        and not re.search(r'\b(base|military installation|federal installation|restricted area|mclb)\b', normalized)
    ):
        forbidden.append('18 USC 1382')
...
def _apply_result_overlays(results: list[LegalMatch], analysis: QueryAnalysis, source: str) -> list[LegalMatch]:
    ...
    for index, code in enumerate(required_codes):
        if code in by_code:
            ...
            _, scenario_reasons, scenario_terms = _scenario_supported_adjustment(_build_entry_profile(existing.entry), analysis)
            by_code[code] = LegalMatch(
                entry=existing.entry,
                score=max(existing.score, seed - min(index * 3, 12)),
                reasons=tuple(_ordered_unique(existing.reasons + scenario_reasons + ('matched likely core reference path',))),
                matched_terms=tuple(_ordered_unique(existing.matched_terms + scenario_terms)),
            )
            continue
        entry = pool.get(code)
        if not entry:
            continue
        _, scenario_reasons, scenario_terms = _scenario_supported_adjustment(_build_entry_profile(entry), analysis)
        by_code[code] = LegalMatch(
            entry=entry,
            score=max(56, seed - min(index * 4, 16)),
            reasons=tuple(_ordered_unique(scenario_reasons + ('matched likely core reference path',))),
            matched_terms=scenario_terms,
        )
...
def _finalize_results(query: str, raw_results: list[LegalMatch]) -> list[LegalMatch]:
    ...
    for item in raw_results:
        ...
        matched_terms = tuple(_ordered_unique(item.matched_terms + tuple(sorted(query_terms & entry_terms))))[:12]
