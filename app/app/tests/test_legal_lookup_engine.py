...
def test_plain_language_officer_scenarios_rank_supported_law_paths():
    cases = [
        (
            'A man came back onto base after being told by police not to return.',
            {'18 USC 1382'},
            {'OCGA 40-6-391'},
        ),
        (
            'Victim said husband pushed her and took her phone.',
            {'OCGA 16-5-23', 'Article 128b'},
            {'OCGA 40-6-241'},
        ),
        (
            'Marine refused a lawful order from his NCO.',
            {'Article 92', 'Article 91'},
            {'18 USC 1382'},
        ),
        (
            'Subject had marijuana inside the vehicle at the gate.',
            {'OCGA 16-13-75', 'OCGA 16-13-30', 'Article 112a'},
            {'18 USC 1382', 'OCGA 40-6-270'},
        ),
        (
            'Person entered a warehouse after hours without permission.',
            {'OCGA 16-7-21', 'OCGA 16-7-22'},
            {'18 USC 1382', '18 USC 930'},
        ),
        (
            'Suspect threatened victim by text message.',
            {'OCGA 16-5-102', '18 USC 2261A'},
            {'OCGA 40-6-241'},
        ),
        (
            'Subject stole items from the PX and passed all points of sale.',
            {'OCGA 16-8-14', 'OCGA 16-8-2', '18 USC 641', 'Article 121'},
            {'18 USC 1382'},
        ),
        (
            'Drunk Marine fighting in barracks.',
            {'Article 112', 'Article 128'},
            {'OCGA 40-6-391', 'Article 120'},
        ),
    ]
    for query, expected_codes, bad_codes in cases:
        results = search_entries(query, 'ALL')
        codes = [item.entry.code for item in results[:6]]
        assert expected_codes & set(codes), (query, codes)
        assert not (bad_codes & set(codes[:3])), (query, codes)
        assert any(reason.startswith('matched fact:') or 'full statute/order text' in reason for reason in results[0].reasons), (
            query,
            results[0].reasons,
        )
