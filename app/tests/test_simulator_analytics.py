...
def _state(run_id='S003-123456789', interviews=1, preserve=True, interventions=0):
    actions = [
...
def test_run_summary_and_comparison_use_observed_actions_not_a_score():
    first = _state('S003-111111111', interviews=1, preserve=False, interventions=2)
...
def test_cross_call_patterns_remain_advisory():
    analytics = aggregate_patterns([
        _state('S003-1', interviews=1, preserve=False, interventions=1),
    ...
    with app.app_context():
        ...
        if len(existing) < 2:
            for index, state in enumerate((
                _state('S003-333333331', interviews=1, preserve=False, interventions=1),
