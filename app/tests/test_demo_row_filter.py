...
def test_demo_rows_are_hidden_from_normal_requests(monkeypatch):
    ...
    with app.app_context():
        real_user = User(username="real.officer", role=ROLE_PATROL_OFFICER, active=True, pending_approval=False)
        demo_user = User(username="demo.officer", role=ROLE_PATROL_OFFICER, active=True, pending_approval=False, is_demo=True)
        real_form = Form(title="Official Real Form", file_path="forms/real.pdf", is_active=True)
        demo_form = Form(title="DEMO Sample Form", file_path="demo/forms/sample.pdf", is_active=True, is_demo=True)
        db.session.add_all([real_user, demo_user, real_form, demo_form])
        db.session.commit()

    with app.test_request_context("/forms"):
        form_titles = [row.title for row in Form.query.order_by(Form.title.asc()).all()]
        usernames = [row.username for row in User.query.order_by(User.username.asc()).all()]
...
def test_demo_rows_are_visible_when_demo_mode_is_active(monkeypatch):
    ...
    with app.app_context():
        db.session.add_all([
            ...
            Form(title="DEMO Sample Form", file_path="demo/forms/sample.pdf", is_active=True, is_demo=True),
        ])
        db.session.commit()
